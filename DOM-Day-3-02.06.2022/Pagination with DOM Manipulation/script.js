...
function navigateTo(index) {
  ...
  pageIndex = index;
  let temp = [...arrayItems].splice(pageIndex * pageSize, pageSize);
  temp.forEach((n) => {
    let row = `<tr><td>${n.id}</td><td>${n.name}</td><td>${n.email}</td></tr>`;
    rows += row;
  });
...
function generateButtons() {
  let numberOfButtons = Math.ceil(arrayItems.length / pageSize);
  let buttons = "";
  if (numberOfButtons > 5) {
    buttons += `<button id="first" onclick="navigateTo(${0})" name='button'> First </button>`;
    let max = 5;
    let min = 0;
    if (pageIndex > 2) {
      min = pageIndex - 2;
      max = pageIndex + 3;
      if (max >= numberOfButtons) {
        max = numberOfButtons;
        min = numberOfButtons - 5;
      }
    }
    if (min != 0) {
      buttons += "....";
    ...
    for (i = min; i < max; i++) {
      if (i == min && pageIndex != 0) {
        buttons += `<button id="previous" onclick="navigateTo(${
          pageIndex - 1
        })" name='button'> Previous </button>`;
      }
      buttons += `<button id="${i}" onclick="navigateTo(${i})" name='button'>${
        i + 1
      }</button>`;
      if (i == max - 1 && pageIndex != 99) {
        buttons += `<button id="next" onclick="navigateTo(${
          pageIndex + 1
  ...
  if (numberOfButtons > 5) {
    buttons += `<button id="last" name='button' onclick="navigateTo(${
      numberOfButtons - 1
    })" > Last </button>`;
  }
  buttons += `<span class="pull-right">displaying  ${
    pageIndex * pageSize + 1
  } of ${arrayItems.length} records </span>`;
